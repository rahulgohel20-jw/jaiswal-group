 ...
 * Category row. Expects `category` to optionally carry:
 *   code, totalAssets, createdDate, activity: [{ title, detail, time }]
 * Falls back to sensible defaults so it also works against the
 * lightweight rows used in AssetCategory.js today.
 */
const AssetCategoryDetailsModal = ({ isOpen, onClose, category }) => {
    ...
    const {
        ...
        createdDate = '—',
        activity = [
            { title: 'Category Updated', detail: 'Description was modified by Admin', time: 'Recently' },
            { title: 'Category Created', detail: 'Initial setup', time: createdDate },
        ],
    } = category;
