    ...
    return (
        <div className="min-h-screen bg-[#F7F8FA] px-6 py-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                <StatCard icon={<Boxes size={18} />} iconBg="bg-[#D5E3FF]" iconColor="text-[#00376C]" label="TOTAL" title="Total Departments" value={stats.total} />
                <StatCard icon={<CheckCircle2 size={18} />} iconBg="bg-green-100" iconColor="text-green-700" label="ACTIVE" title="Active Units" value={String(stats.active).padStart(2, '0')} />
                <StatCard icon={<XCircle size={18} />} iconBg="bg-gray-100" iconColor="text-gray-500" label="PAUSED" title="Inactive Units" value={String(stats.inactive).padStart(2, '0')} />
...
const StatCard = ({ icon, iconBg, iconColor, label, title, value }) => (
    <div className="bg-white border border-[#E5E7EB] rounded-xl p-4 flex items-start justify-between">
        <div>
            <div className={`w-9 h-9 rounded-lg ${iconBg} ${iconColor} flex items-center justify-center mb-3`}>{icon}</div>
